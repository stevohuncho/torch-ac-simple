...
def test_a2c_agent_creation() -> torch_ac_simple.Agent:
    return torch_ac_simple.Agent("MiniGrid-Empty-6x6-v0", "a2c_test")

def test_a2c_agent_train(a: torch_ac_simple.Agent):
    a.train(1000, "a2c", log_interval=10, save_interval=10)

def test_a2c_agent_eval(a: torch_ac_simple.Agent):
    a.eval(100)

def test_ppo_agent_creation() -> torch_ac_simple.Agent:
    return torch_ac_simple.Agent("MiniGrid-Empty-6x6-v0", "ppo_test")

def test_ppo_agent_train(a: torch_ac_simple.Agent):
    a.train(1000, "ppo", log_interval=10, save_interval=10)

def test_ppo_agent_eval(a: torch_ac_simple.Agent):
    a.eval(100)
